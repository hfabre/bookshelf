...
Release flow: the `publish` job in `.github/workflows/ci.yml` builds the image from the `Dockerfile` and pushes it to GHCR once the rest of CI is green — a version tag `vX.Y.Z` publishes `:X.Y.Z`/`:X.Y`/`:latest`, and every push to `main` publishes `:nightly`. Deploy is bumping the image tag in the compose file and `docker compose pull && up -d --wait`.
