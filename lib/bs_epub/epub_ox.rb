...
module BsEpub
  ...
  class EpubOx
    ...
    NODE_ACCESS = {
      ...
      authors: lambda { |metadata_node| metadata_node.nodes.select { _1.value == "dc:creator" } },
    ...
    attr_reader :path_or_io, :zip, :opf_content, :epub_version, :metadata_node, :custom_metas, :failure_reason
    ...
    def load_book
      ...
      @zip =
        ...
        elsif path_or_io.is_a?(String) && File.exist?(path_or_io)
          Zip::File.new(path_or_io)
        else
    ...
    def replace_cover!(cover)
      ...
      if new_name != File.basename(cover_path)
      ...
      metadata.each do |k, v|
        if (node_access = NODE_ACCESS[k])
          ...
          if node && k == :authors
            ...
            metadata_node.nodes.delete(node)
    ...
    def root_meta_node
      opf_content.root.nodes.find { |el| el.is_a?(Ox::Element) && el.value == "metadata" }
    end
    ...
    def find_cover_filename_fallback
      manifest_node = opf_content.root.nodes.find { |n| n.value == "manifest" }
      return nil unless manifest_node

      cover_patterns = [ /cover\.(jpe?g|png)/i, /^cover$/i ]

      manifest_node.nodes.each do |node|
        next unless node.attributes && node.attributes[:href]
