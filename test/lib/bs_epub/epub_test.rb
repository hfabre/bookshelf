...
describe BsEpub::Epub do
  ...
  describe "#update_mt!" do
    ...
    it "updates epub metadata" do
      ...
      _(subject.mt_hash[:authors]).must_equal [ new_metadata[:authors] ]
  ...
  describe "#mt_hash" do
    describe "valid epub" do
      subject { BsEpub::Epub.new(File.read("test/fixtures/files/valid.epub")) }

      it "reads metadata from valid epub" do
        result = subject.mt_hash

        _(result[:title]).must_equal "Légendes espagnoles"
        _(result[:authors]).must_equal [ "Gustavo-Adolfo Bécquer" ]
      ...
      describe "series metadata" do
        describe "calibre series" do
          subject { BsEpub::Epub.new(File.read("test/fixtures/files/calibre_serie.epub")) }
        ...
        describe "epub 3.0 series" do
          subject { BsEpub::Epub.new(File.read("test/fixtures/files/epub_3_serie.epub")) }
    ...
    describe "invalid epub" do
      subject { BsEpub::Epub.new(File.read("test/fixtures/files/missing_container.epub")) }

      it "render default metadata" do
        result = subject.mt_hash

        _(result[:title]).must_be_nil
        _(result[:authors]).must_be_nil
